...
func TestSortedSet_ZRank(t *testing.T) {
	zset := InitZSet()
	key := "myzset"
	// 先比较 score 再比较 key 大小
	r1_1 := zset.ZRank(key, "ced") // 4
	r1_2 := zset.ZRank(key, "acd") // 0
	r1_3 := zset.ZRank(key, "bcd") // 1
	r1_4 := zset.ZRank(key, "acc") // 6
	r1_5 := zset.ZRank(key, "mcd") // 3
	r1_6 := zset.ZRank(key, "ccd") // 5
	r1_7 := zset.ZRank(key, "ecd") // 2
	fmt.Println(r1_1)
	fmt.Println(r1_2)
	fmt.Println(r1_3)
	fmt.Println(r1_4)
